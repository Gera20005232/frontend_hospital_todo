    ...
    if (selectedVirus && mes) {
      try {
        const response = await axios.post(
          `http://localhost:8089/v1/vigilancia/calcular-prediccion/MilpaAlta/${selectedVirus}/${mes}`
        );
...
const handleAceptarBacterias = async () => {
  ...
  if (selectedBacterias && mes) {
    try {
      const response = await axios.post(
        `http://localhost:8089/v1/vigilancia/calcular-prediccion-bacterias/MilpaAlta/${selectedBacterias}/${mes}`
      );
      console.log("Respuesta del servidor:", response.data);
...
/** */
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:8089/v1/vigilancia/buscar/MilpaAlta");
